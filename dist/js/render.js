...
module.exports = function (calendar) {
    ...
    //var normal3 = d3.scaleLinear()
    //    .domain([0, days * 11.5])
    //    .range([0, 8*12*Math.PI]);
	//
	//
	//
    //    months.forEach(function(month, k, arr) {
	...
	//
	//
    //        for (i = 0; i < month.days*16; i++) {
	//
    //            var x = Math.round(10*(center.x + (radius * 0.5  - 40  * (Math.sin(normal3(i))) ) * Math.cos(angle * j /2)) ) / 10;
    //            var y = Math.round(10*(center.y - (radius * 0.5  - 40  * (Math.sin(normal3(i))) ) * Math.sin(angle * j /2)) ) / 10;
	//
    //            lineData2.push({
    //                x: x,
    //                y: y
	...
	//
    //    var base = calendar.append("path")
    //        .attr("d", lineFunction2(lineData2))
    //        .classed("mj-calendar__line", true)
    //        .classed("_additional", true);
	//
	//
    //    j = 0;
        ...
        months.forEach(function(month, k, arr) {
            ...
            for (i = 0; i < month.days; i++) {
                ...
                x = Math.round(10*(center.x + (radius - 40 * (Math.cos(normal(i))*Math.cos(normal(i)) )) * Math.sin(angle * j))) / 10;
                y = Math.round(10*(center.y - (radius - 40 * (Math.cos(normal(i))*Math.cos(normal(i)) )) * Math.cos(angle * j))) / 10;
                ...
                textBox.append("text")
                    ...
                    function() {
                        if (j < days/2) {
                            return 'rotate(' + Math.round(10*( ( (j) * (360 / days) + 270) )) / 10 + ', 0, 0)';
                        } else {
                            return 'rotate(' + Math.round(10*( ( (j) * (360 / days) + 90) )) / 10 + ', 0, 0)';
                        }
                    }
        ...
        for (i = 0; i < 12; i++) {
            ...
            var x = center.x + radius * (Math.sin(angle * days / 12 * (i+0.5))) * 0.74;
            var y = center.y - radius * (Math.cos(angle * days / 12 * (i+0.5))) * 0.74;

            month.append("text")
                .attr('x', x)
                .attr('y', y)
                .attr('transform', 'rotate(' + ( (i+0.5) * (360/12) ) + ', ' + x + ', ' + y + ')')
